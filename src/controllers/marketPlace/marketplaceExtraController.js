...
const addReview = asyncHandler(async (req, res) => {
    ...
    const user_id = req.user._id;
    const review = await marketplaceService.addReview(user_id, product_id, rating, comment);
...
const createCart = asyncHandler(async (req, res) => {
    const user_id = req.user._id;
    const cart = await marketplaceService.createCart(user_id);
    res.status(201).json({ success: true, cart });
});

const addItemToCart = asyncHandler(async (req, res) => {
    const { cart_id, product_id, quantity, price } = req.body;
    const item = await marketplaceService.addItemToCart(cart_id, product_id, quantity, price);
    res.status(201).json({ success: true, item });
});
